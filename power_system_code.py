# -*- coding: utf-8 -*-
"""Power System Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1coRx5nDu4yKs3bcSXHblPKjKkUj62Ae_
"""
...
def gauss_seidel_power_flow(bus_data, line_data, max_iter=100, tol=1e-6):
    ...
    V = np.array([bus[4] * np.exp(1j * np.radians(bus[6])) for bus inbus_data])
    ...
    for iteration in range(max_iter):
        ...
        for i in range(nb):
            ...
            if bus_types[i] == PV:
                # Fix magnitude of V to specified
...
def calculate_power_losses(V, Ybus):
    ...
    # Load power is sum of negative P (loads are negative)
...
if __name__ == "__main__":
    # bus_data format: [bus_no, type, P(pu), Q(pu), V_spec(pu), V_init_mag, V_init_angle_deg]
    bus_data = [
        [1, SLACK, 0, 0, 1.06, 1.06, 0],
        [2, PV, 0.5, 0, 1.045, 1.0, 0],
        [3, PQ, -0.6, -0.3, 1.0, 1.0, 0]
    ]
    # line_data format: [from_bus, to_bus, R(pu), X(pu), B(pu)]
    line_data = [
        [1, 2, 0.02, 0.06, 0.03],
        [1, 3, 0.08, 0.24, 0.025],
        [2, 3, 0.06, 0.18, 0.02]
